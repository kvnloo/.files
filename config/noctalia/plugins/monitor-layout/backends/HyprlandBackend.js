...
function parseOutputs(rawText) {
  try {
    ...
    for (var index = 0; index < parsed.length; index++) {
      ...
      var logicalWidth = Math.max(1, Math.round((rotated ? height : width) / scale));
      var logicalHeight = Math.max(1, Math.round((rotated ? width : height) / scale));
      var currentModeId = modeIdFromRes(width, height, refresh);
      var modes = buildAvailableModes(monitor.availableModes || [], width, height, refresh);
...
function buildApplyCommand(outputs, cfg, defaults) {
  ...
  for (var index = 0; index < outputs.length; index++) {
    ...
    // Preserve the complete monitor rule, including rotation.
    var command =
      ...
      shellQuote(position) + "," +
      shellQuote(scale);
    if (Number(output.transform || 0) !== 0) {
      command += ",transform," + shellQuote(String(output.transform));
    }
...
function buildConfigFileContent(outputs) {
  ...
  for (var index = 0; index < outputs.length; index++) {
    ...
    if (Number(output.transform || 0) !== 0) {
      line += ",transform," + String(output.transform);
    }
    lines.push(line);
  }
...
function buildAvailableModes(availableModes, currentWidth, currentHeight, currentRefresh) {
  var modes = [];
  var seen = {};
  ...
  pushMode(currentWidth, currentHeight, currentRefresh, true);
  for (var index = 0; index < availableModes.length; index++) {
    var match = String(availableModes[index]).match(/^(\d+)x(\d+)@([0-9.]+)Hz$/);
    if (!match) {
      continue;
    }
    var width = Number(match[1]);
    var height = Number(match[2]);
    var refresh = Number(match[3]);
    pushMode(
      width,
      height,
      refresh,
      width === currentWidth && height === currentHeight &&
        Math.abs(normalizeRefreshRate(refresh) - normalizeRefreshRate(currentRefresh)) < 0.02
    );
  }
  return modes;
}
