#!/usr/bin/env bash
# expand-root-from-rescue.sh v2.0.0
...
readonly FSTAB_TOOL="$SCRIPT_DIR/rescue_expand_fstab.py"
readonly RECEIPT_TOOL="$SCRIPT_DIR/rescue_expand_receipt.py"
...
Receipts must be stored on a separate persistent physical disk (never target p1-p4).
...
[[ -f "$FSTAB_TOOL" && ! -L "$FSTAB_TOOL" ]] || die "fstab contract module missing or unsafe"
[[ -f "$RECEIPT_TOOL" && ! -L "$RECEIPT_TOOL" ]] || die "receipt topology module missing or unsafe"
...
parts={x.get('name','').rsplit('p',1)[-1]:x for x in disk.get('children',[])}
expected={
...
already=(set(parts)=={'1','3','4'} and
         ...
         parts['1'].get('partuuid')=='5fed8d1d-f6a6-44d3-9608-0408425a9383' and parts['1'].get('type')=='part' and
         parts['3'].get('start')==281020416 and parts['3'].get('size')==695752719*512 and
         parts['3'].get('fstype')=='xfs' and parts['3'].get('uuid')=='f70404dd-c13b-4070-8d7c-d6d5e91db153' and parts['3'].get('type')=='part' and
         parts['3'].get('partuuid')=='ffbd4444-5995-4ed1-abd1-c0ac108848ef' and
         parts['4'].get('start')==2048 and parts['4'].get('size')==4194304*512 and
         parts['4'].get('fstype')=='vfat' and parts['4'].get('uuid')=='DBCE-C10E' and parts['4'].get('type')=='part' and
         parts['4'].get('partuuid')=='11878bd8-0fee-471e-90b7-3c18db40fa85')
if already:
    for n in ('1','3','4'):
        req(parts[n].get('path')==f'/dev/nvme0n1p{n}',f'grown p{n} path identity drift')
        req(parts[n].get('name')==f'nvme0n1p{n}',f'grown p{n} name identity drift')
        req(not any(parts[n].get('mountpoints') or []),f'grown p{n} is mounted/in use')
...
for n,(start,size,fs,uuid,partuuid) in expected.items():
    x=parts[n]
    req(x.get('path')==f'/dev/nvme0n1p{n}' and x.get('name')==f'nvme0n1p{n}',f'p{n} path identity drift')
...
prepare_receipts() {
  ...
  local out_source out_type avail resolved
  out_source=$(findmnt -rn -o SOURCE --target "$OUTPUT_DIR")
  out_type=$(findmnt -rn -o FSTYPE --target "$OUTPUT_DIR")
  out_source=${out_source%%\[*} # findmnt appends [subvolume] for some persistent filesystems
  case "$out_type" in tmpfs|overlay|ramfs|squashfs|aufs) die "receipt path is not persistent";; esac
  resolved=$(readlink -f -- "$out_source") || die "cannot canonicalize receipt mount source"
  python3 "$RECEIPT_TOOL" "$resolved" "$EXPECTED_DISK" "$out_type" || die "receipt backing-device contract failed"
  [[ -w "$OUTPUT_DIR" ]] || die "receipt directory is not writable"
...
FSTAB_STAGED="$RECEIPT_DIR/fstab.staged"
cp --preserve=mode,timestamps "$MOUNT_DIR/etc/fstab" "$FSTAB_BACKUP"
python3 "$FSTAB_TOOL" check-original "$FSTAB_BACKUP" || die "fstab original contract failed"
python3 "$FSTAB_TOOL" stage "$FSTAB_BACKUP" "$FSTAB_STAGED" || die "fstab staging failed"
python3 "$FSTAB_TOOL" check-final "$FSTAB_BACKUP" "$FSTAB_STAGED" || die "fstab staged preservation failed"
...
[[ "$(validate_geometry "$RECEIPT_DIR/lsblk-after-table.json")" == already-grown ]] || \
  die "post-table p1/p3/p4 identity contract failed"
...
python3 "$FSTAB_TOOL" check-original "$MOUNT_DIR/etc/fstab" || die "fstab original contract changed"

GROW_STARTED=1
xfs_growfs "$MOUNT_DIR" >"$RECEIPT_DIR/xfs-growfs.txt" 2>&1
STATE=grown; printf '%s\n' "$STATE" >"$RECEIPT_DIR/checkpoint"

# Install only the exact transform staged and validated before GROW_STARTED.
python3 "$FSTAB_TOOL" install "$FSTAB_STAGED" "$MOUNT_DIR/etc/fstab" || die "atomic fstab install failed"
...
python3 "$FSTAB_TOOL" check-final "$FSTAB_BACKUP" "$MOUNT_DIR/etc/fstab" || die "exact final fstab verification failed"
